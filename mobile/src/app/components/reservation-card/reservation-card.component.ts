...
export class ReservationCardComponent {
    ...
    @Input() eventName: string = ''; // was providerName
    ...
    get startTime(): Date {
        return new Date(this.reservation.start_time);
    }

    get endTime(): Date {
        return new Date(this.reservation.end_time);
    }
    ...
    get confirmationStateIcon(): string {
        switch (this.reservation.confirmationState) {
    ...
    get showCheckIn(): boolean {
        ...
        return minutesUntil >= -15 && minutesUntil <= 30 && this.reservation.status === 'BOOKED';
    }
    ...
    handleViewConflict(): void {
        this.onViewConflict.emit(this.reservation.id);
    }
}
